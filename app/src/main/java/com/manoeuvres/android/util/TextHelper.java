...
public class TextHelper {
    ...
    public static String getDurationText(long startTime, long endTime, Resources resources) {
        ...
        if (days > 0) {
            if (days == 1) {
                text = resources.getString(R.string.duration_one_day);
            } else {
                text = days + resources.getString(R.string.duration_days);
            }
        }

        if (hours > 0) {
            if (hours == 1) {
                text += resources.getString(R.string.duration_one_hour);
            } else {
                text += String.valueOf((int) hours) + resources.getString(R.string.duration_hours);
            }
        }

        if (minutes == 0) {
            text += resources.getString(R.string.duration_less_than_a_minute);
        } else if (minutes > 0) {
            if (minutes == 1) {
                text += resources.getString(R.string.duration_minute);
            } else {
